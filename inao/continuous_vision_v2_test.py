# -*- encoding: UTF-8 -*-
# Continuous Vision V2: Advanced version with Object Counting, Selective Alerts, and Number-Based Decisions
# Builds on continuous_vision_test.py (continuous fetching + visualization + YOLO + OCR)
# Adds: Running object counts, speech alerts for priority objects, and decisions based on number detections
...
def continuousVisionProcessingV2(IP, PORT, target_objects=None, max_frames=None):
    """
    Continuous Vision V2: Fetch frames continuously from NAO, display live video, process with YOLO + OCR.
    Adds: Object counting, selective alerts, and number-based decisions.
    Runs indefinitely or until max_frames reached. Press 'q' in video window or Ctrl+C to stop.
    """
    camProxy = ALProxy("ALVideoDevice", IP, PORT)
    tts = ALProxy("ALTextToSpeech", IP, PORT)  # For selective alerts
    resolution = 2  # VGA
    colorSpace = 11  # RGB
    ...
    print(
        "Subscribed to NAO video feed. Processing frames continuously with live visualization and advanced features..."
    )
    if target_objects:
    ...
    print(
        "Advanced features: Object counting, selective alerts, number-based decisions."
    )
    print("Press 'q' in video window or Ctrl+C to stop.")

    frame_count = 0
    object_counts = {}  # For object counting
    try:
        while True:
            ...
            if detected:
                ...
                # Object Counting (accumulate across frames)
                for label, conf in detected:
                    object_counts[label] = object_counts.get(label, 0) + 1
                print("  Running object counts: {}".format(object_counts))

                # Selective Object Alert (speak for priority objects)
                priority_objects = ["person", "sports ball"]
                for label, conf in detected:
                    if label in priority_objects and conf > 0.7:
                        alert_msg = "Alert: I see a {}!".format(label)
                        print("  {}".format(alert_msg))
                        tts.say(alert_msg)
            else:
            ...
            if numbers:
                ...
                # Number-Based Decision Making (decide based on number count)
                if len(numbers) > 2:
                    decision_msg = (
                        "Decision: Multiple numbers detected – activating math mode!"
                    )
                    print("  {}".format(decision_msg))
                    tts.say("Math time!")
            else:
                print(
            ...
            cv2.imshow("NAO Live Vision V2", bgr_image)
        ...
        print(
            "Unsubscribed from video feed. Processed {} frames. Final object counts: {}".format(
                frame_count, object_counts
            )
        )
...
if __name__ == "__main__":
    ...
    continuousVisionProcessingV2(
        IP, PORT, target_objects=target_objects, max_frames=None
    )
